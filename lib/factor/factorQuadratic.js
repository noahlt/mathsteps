...
const getQuadraticCoefficients = require('./getQuadraticCoefficients');
...
function factorQuadratic(node) {
  // get a, b and c
  const coefficients = getQuadraticCoefficients(node);
  if (!coefficients) {
    return Node.Status.noChange(node);
  }
  const {aValue, bValue, cValue, symbol, negate} = coefficients;
